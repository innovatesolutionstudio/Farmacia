...
function obtenerMensajeReporte(id) {
  return new Promise((resolve, reject) => {
    connection.query(
      "SELECT ID_Mensaje, Texto FROM mensajes WHERE ID_Mensaje = ?",
      [id],
  ...
  connection.query(
    ...
    async (error, results) => {
      ...
      doc
        .fontSize(12)
        .font("Helvetica-Bold")
        .text(`Fecha de reporte: ${fecha}`, 55, doc.y + 10);
      doc.moveDown();
...
function crearTablaVentas(doc, dataVentas) {
  ...
  const cellWidth = (doc.page.width - 150) / tableHeaders.length; // Ajustar ancho de las celdas
  ...
  dataVentas.forEach((venta, index) => {
    ...
    if (y + fontSize + cellPadding * 2 > doc.page.height - 50) {
      doc.addPage(); // Añadir nueva página si se sobrepasa el límite de la página
      y = tableTop; // Reiniciar la posición vertical
    ...
    drawRow(
      doc,
      [venta.Codigo_Caja, venta.Total_Venta, fechaVenta],
      y,
      fillColor,
      textColor
    );
...
function drawRow(doc, rowData, y, fillColor, textColor) {
  const cellWidth = (doc.page.width - 150) / rowData.length; // Ajustar el ancho de cada celda
  rowData.forEach((cellData, index) => {
    const cellX = 100 + index * cellWidth;
    doc
      .rect(cellX, y, cellWidth, fontSize + cellPadding)
      .fillAndStroke(fillColor, "#000"); // Dibuja el rectángulo de la celda con color de fondo
    doc.fillColor(textColor).fontSize(12);
    doc.text(cellData != null ? cellData.toString() : "", cellX, y + 5, {
      width: cellWidth,
      align: "center",
    });
  });
...
function crearTablaVentas(doc, dataVentas) {
  ...
  dataVentas.forEach((venta, index) => {
    ...
    // Dibujar cada fila de datos, pasando fontSize como parámetro
    drawRow(
      doc,
      [venta.Codigo_Caja, venta.Total_Venta, fechaVenta],
      y,
...
function drawRow(doc, rowData, y, fillColor, textColor, fontSize, cellPadding) {
  const cellWidth = (doc.page.width - 150) / rowData.length; // Ajustar el ancho de cada celda
  rowData.forEach((cellData, index) => {
    const cellX = 100 + index * cellWidth;
    doc
...
function crearTablaVentas(doc, dataVentas) {
  ...
  // Dibujar encabezado de la tabla solo en la primera página
  doc.fillColor(headerColor).fontSize(fontSize).font("Helvetica-Bold");
  tableHeaders.forEach((header, index) => {
    doc.text(header, 100 + index * cellWidth, y, {
      width: cellWidth,
  ...
  dataVentas.forEach((venta, index) => {
    ...
    if (y + fontSize + cellPadding * 2 > doc.page.height - 50) {
      doc.addPage(); // Añadir nueva página si se sobrepasa el límite de la página
      y = 50; // Posicionar `y` cerca de la parte superior para continuar sin espacio en blanco
    }
