...
router.get("/compras", function (req, res) {
  if (req.session.loggedin) {
    ...
    if (fechaInicio && fechaFin) {
      ...
      sql += ` ORDER BY c.Fecha_Compra DESC LIMIT 200`;
    }
...
router.get("/compras_papeleria", function (req, res) {
  if (req.session.loggedin) {
    const { fechaInicio, fechaFin } = req.query;
    ...
    connection.query(
      ...
      (error, results) => {
        if (error) {
          ...
          res.render("./compras/compras_papeleria", { results: results });
        }
      }
    );
  } else {
...
router.post("/compras/:id?", function (req, res) {
  ...
  switch (opcion) {
    ...
    case "eliminar":
      connection.query(
        "UPDATE compras SET Estado = 2 WHERE ID_Compra = ?",
        id,
        (error, result) => {
          if (error) {
    ...
    case "restaurar":
      connection.query(
        "UPDATE compras SET Estado = 1 WHERE ID_Compra = ?",
        id,
        (error, result) => {
          if (error) {
            console.error("Error al eliminar la compra:", error);
