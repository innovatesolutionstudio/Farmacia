...
router.post("/save_chart_image", (req, res) => {
  ...
  const imagePath = path.join(
    __dirname,
    "graficos_reportes",
    `${shortTitle}_${year}_${timestamp}.png`
  );
  const pdfPath = path.join(
    __dirname,
    "reporte_generado",
    `${shortTitle}_${year}_${timestamp}.pdf`
  );
  ...
  fs.writeFile(imagePath, base64Data, "base64", (err) => {
    ...
    try {
      ...
      doc.fontSize(13).font("Helvetica-Bold").text(title, 130, 130);
      ...
      doc
        .fontSize(13)
        .font("Helvetica-Bold")
        .text("Grafico de " + title, { align: "center" });
      ...
      stream.on("finish", () => {
        ...
        res.json({
          pdfUrl: `/reporte_generado/${shortTitle}_${year}_${timestamp}.pdf`,
        });
      });
    }
  });
...
router.use(
  "/graficos_reportes",
  express.static(path.join(__dirname, "graficos_reportes"))
);
router.use(
  "/reporte_generado",
  express.static(path.join(__dirname, "reporte_generado"))
);
