...
router.get("/impre_departamentos", async (req, res) => {
  ...
  const stream = fs.createWriteStream(
    `./impresiones/Impre_Departamentos-${fecha}.pdf`
  );
...
function crearTablaDepartamentos(doc, dataDepartamentos) {
  ...
  function drawTable() {
    ...
    for (let i = 0; i <= dataDepartamentos.length + 1; i++) {
      doc
        .moveTo(100, y + i * (fontSize + cellPadding))
        .lineTo(
          100 + tableHeaders.length * cellWidth,
          y + i * (fontSize + cellPadding)
        )
        .stroke();
    }
    ...
    for (let i = 0; i <= tableHeaders.length; i++) {
      doc
        .moveTo(100 + i * cellWidth, y)
        .lineTo(
          100 + i * cellWidth,
          y + (fontSize + cellPadding) * (dataDepartamentos.length + 1)
        )
        .stroke();
    }
    ...
    dataDepartamentos.forEach((departamento, index) => {
      ...
      drawRow(
        ...
        false,
        "black"
      ); // Dibujar el resto de las filas sin negrita
      y += fontSize + cellPadding;
    });
  }
  ...
  function drawRow(rowData, y, bold, backgroundColor) {
    ...
    rowData.slice(0, 2).forEach((cellData, index) => {
      // Iterar solo sobre las dos primeras celdas
      ...
      if (bold) {
        doc
          ...
          .fontSize(fontSize)
          .fillColor("#fff")
          .text(cellData.toString(), cellX, y + (fontSize + cellPadding) / 2, {
            width: cellWidth,
            align: "center",
          });
      } else {
...
function agregarFirmas(doc, yPos) {
  ...
  doc.image("assets/images/firmas/firma_A.png", 140, yPos + 35, {
    width: 120,
    height: 120,
  });
  // Insertamos la segunda imagen
  doc.image("assets/images/firmas/firma_f.png", 370, yPos + 30, {
    width: 120,
    height: 120,
  });
