...
router.post("/nueva_compra", async (req, res) => {
  ...
  try {
    ...
    const nuevaCompraQuery =
      "INSERT INTO compras (Fecha_Compra, ID_Proveedor, ID_Empleado, ID_Sucursal) VALUES (?, ?, ?, ?)";
    ...
    connection.query(nuevaCompraQuery, compraValues, (error, insertCompra) => {
      ...
      const detallesCompraQuery =
        "INSERT INTO detalles_compra (ID_Compra, ID_Producto, Cantidad_Empaque, Cantidad_Unitario, Fecha_Vencimiento, Precio_Unitario, ID_Unidad_Empaque) VALUES ?";
      connection.query(
        detallesCompraQuery,
        ...
        async (error) => {
          ...
          for (const producto of productos) {
            ...
            if (inventarioExistente) {
              ...
              const insertarInventarioQuery =
                "INSERT INTO inventario (ID_Producto, Cantidad, ID_Sucursal) VALUES (?, ?, ?)";
              connection.query(
                insertarInventarioQuery,
                [ID_Producto, cantidadEmpaque * cantidadUnitario, ID_Sucursal],
                (error) => {
                  if (error) {
                    throw error;
                  }
